...
import { PublicTourKeypointsComponent } from "src/app/feature-modules/tour-authoring/keypoints/public-tour-keypoints/public-tour-keypoints.component";
import { PublicFacilitiesComponent } from "src/app/feature-modules/tour-authoring/facilities/public-facilities/public-facilities.component";
...
import { ChallengeLocationDialogComponent } from "src/app/feature-modules/tour-execution/challenges/challenge-location-dialog/challenge-location-dialog.component";
import { ChallengeMiscDialogComponent } from "src/app/feature-modules/tour-execution/challenges/challenge-misc-dialog/challenge-misc-dialog.component";
import { ChallengeSocialDialogComponent } from "src/app/feature-modules/tour-execution/challenges/challenge-social-dialog/challenge-social-dialog.component";
import { TourCreationComponent } from "src/app/feature-modules/tour-authoring/tours/tour-creation/tour-creation.component";
...
export class MapViewComponent implements OnInit {
  ...
  showTourOnMap(tour: Tour) {
    ...
    switch (this.user.role) {
      case 'author': 
      {
        ...
        this.mapComponent.routingControl = L.Routing.control({
          waypoints: this.mapComponent.markers.map((marker) => marker.getLatLng()),
          show: false,
          lineOptions: {
            addWaypoints: false,
        ...
        if (routingControlElement) {
          (routingControlElement as HTMLElement).style.display = 'none';
        }
      }
        break;
      case 'tourist':
        ...
        if (this.mapComponent.routingControl) {
          this.mapComponent.map.removeControl(this.mapComponent.routingControl);
        }
        this.mapComponent.routingControl = L.Routing.control({
          waypoints: this.mapComponent.markers.map((marker) => marker.getLatLng()),
          show: false,
  ...
  private setTourDistanceCrossed() {
    if (this.activeSession != null) {
      ...
      if (this.mapComponent.routingControl) {
          if (this.mapComponent.routingControl.getWaypoints().length > 0) {
            // Remove existing routing control from the map
            this.mapComponent.map.removeControl(this.mapComponent.routingControl);
        }
      }
      this.mapComponent.routingControl = L.Routing.control({
